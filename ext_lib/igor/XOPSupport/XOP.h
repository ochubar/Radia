...
/*	Private XOP information maintained by Igor
	Although this is private, it is used in XOPSupport.c and XOPSupportMac.c.
	Therefore this structure can not be changed without breaking existing XOPs.
*/
struct XOPStuff {					// Structure of private bookkeeping info
	...
	unsigned char system;			// Code for development system
		...
		short xopRefNum;			// XOP's file reference number
		short curResFile;			// XOP's curResFile
		Ptr worldPtr;				// Obsolete
		...
		UInt32 connID;				// ConnectionID for code fragment or zero
		
		// HR, 030122: Support Mach XOP.
		CFBundleRef bundleInstance;
...
#undef DUPLICATE	// Conflicts with nb30.h
