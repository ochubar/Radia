...
class CWinContViewer3D : public CWinCont {
	...
	double m_Aspect; //OC050109
	double m_DepthNear, m_DepthFar;
	double m_Depth, m_DepthMotionQuanta;
...
public:
	...
	CWinContViewer3D(double** VertCoordArr, int* NvArr, int** VertIndArr, int** LenArr, float** ColorsArr, int* NpArr, char* WinTitle);
	CWinContViewer3D() 
	{
		InitViewingParams();
		SetupRotInit();
	...
	static void DrawArbPolygObjects(GLenum Mode, double* VertCoord, int Nv, int* VertInd, int* Lengths, float* Colors, int Npg, double* NormCoord, bool& NormalsWereDefined); //OC050109
