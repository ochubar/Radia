...
{//wrap for Multi-Treading entry (on Win32)
...
void CSimpleGraph::StartNewWinApp(CHWinCont& hWinCont)
{
...
void CSimpleGraph::StartNewWin(CHWinCont& hWinCont)
{
	...
	if(hWinCont.rep != 0)
	{
		//if(hWinCont.rep->m_xPos >= MinPosX) PosX = hWinCont.rep->m_xPos;
		//if(hWinCont.rep->m_yPos >= MinPosY) PosY = hWinCont.rep->m_yPos;
    ...
    StartNewWin(hWinCont, PosX, PosY, SizeX, SizeY, sm_DefWinTitle);
}

//-------------------------------------------------------------------------

void CSimpleGraph::ProcIdle()
{//this function starts new windows after creation of the first window
	...
	if(sm_SomethingToCreate)
	{
		...
		{
			int AmOfWinToCreate = sm_WinToCreateContList.size();
...
{
	int DefaultFileToScreenResolRatio = 1;
	if(WinID == 0) return;
	...
	//int SaveWidth = CurWidth;
	//int SaveHeight = CurHeight;
...
//	HGDIOBJ res00 = SelectObject(hDC, Aux_hBMP);
//
...
//	SwapRB = true;
//	void *BitData=0;
//	HBITMAP hBMP = CreateDIBSection(hDC, &bmi, DIB_RGB_COLORS, &BitData, NULL, NULL);
...
//	}
//
//    bool bResult = SetPixelFormat(hDC, nPixelFormat, &pfd); // Set the pixel format.
...
//    HGLRC hGLRC = wglCreateContext(Old_hDC);
//    wglMakeCurrent(Old_hDC, hGLRC);
//#endif

	//OSMesaContext ctx = OSMesaCreateContextExt(OSMESA_RGBA, CurColorDepth, 0, 0, NULL);
	//if(!OSMesaMakeCurrent(ctx, pData, GL_UNSIGNED_BYTE, SaveWidth, SaveHeight)) return;
...
bool CSimpleGraph::SaveImageToBMP(GLRGBQUAD *pData, int Width, int Height, const char *filename)
{
	...
	long bytesWritten = fwrite(&fHeader.bfType, 1, 2, bmpFile); if(bytesWritten != 2) { fclose(bmpFile); return false;}
	bytesWritten = fwrite(&fHeader.bfSize, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
	bytesWritten = fwrite(&fHeader.bfReserved1, 1, 2, bmpFile); if(bytesWritten != 2) { fclose(bmpFile); return false;}
	bytesWritten = fwrite(&fHeader.bfReserved2, 1, 2, bmpFile); if(bytesWritten != 2) { fclose(bmpFile); return false;}
	bytesWritten = fwrite(&fHeader.bfOffBits, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
    ...
    bytesWritten = fwrite(&iHeader.biSize, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biWidth, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biHeight, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biPlanes, 1, 2, bmpFile); if(bytesWritten != 2) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biBitCount, 1, 2, bmpFile); if(bytesWritten != 2) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biCompression, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biSizeImage, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biXPelsPerMeter, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biYPelsPerMeter, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biClrUsed, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}
    bytesWritten = fwrite(&iHeader.biClrImportant, 1, 4, bmpFile); if(bytesWritten != 4) { fclose(bmpFile); return false;}

    AuxSwapRB(pData, Width, Height);

	bytesWritten = fwrite(pData, 1, length, bmpFile);
