...
/** Attempts to find a minimum of a function of many variables.
@param MethNo [in] minimization method number: 
		1- random search
		10- simplex method
		11- combined random search and simplex method
@param NumDim [in] number of variables in the scaler function to be minimized
@param pExtFunc [in] pointer to the scalar function to minimized;
		this function has the form: (*pExtFunc)(double* pArgs, double* pArgsMin, double FMin), where:
@param pArgs is the array of the function variables (of NumDim length),
@param pArgsMin is the (output) array of the function variables (of NumDim length) that correspond to a current (intermediate) minimum found so far in the process of minimization,
@param FMin is the (output) current (intermediate) minimum value found so far in the process of minimization;
		The pArgsMin, FMin parameters are added for communicating intermediate results of the minimization process to the external environment(?).
@param pArgsLower [in] is the array (of NumDim length) containing lower limit values in the function variables to use at the minimization
@param pArgsUpper [in] is the array (of NumDim length) containing upper limit values in the function variables to use at the minimization
@param ArgRelPrec [in] is the relative precision of the function variables at the minimum value to be found
@param MaxIter [in] is the maximal number of the function calls to be made during the minimum search
@param pArgsMin [out] is the array (of NumDim length) containing the values in the function variables corresponding to the minimum found
@param pfMin [out] is a pointer via which the function minimum value found is returned
@param ErrWarnText [out] error or warning text (to be allocated by calling application)
@return minimum function value found
@author O.C.
*/
...
EXP double CALL gmMinFindMD(int MethNo, int NumDim, double (*pExtFunc)(double* pArgs, double* pArgsMin, double FMin), double* pArgsLower, double* pArgsUpper, double ArgRelPrec, int MaxIter, double* pArgsMin, char* ErrWarnText);
